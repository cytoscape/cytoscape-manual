...
- Hold down the left mouse button on the canvas background and drag the mouse while holding down the ``Shift`` or ``Ctrl`` key (``Command`` on Macs) to select groups of nodes/edges/annotations using a classic rectangular selection area.
- Hold down the left mouse button on the canvas background and drag the mouse while holding down ``Option``+ ``Shift`` to select groups of nodes/edges/annotations using a lasso selection tool.
